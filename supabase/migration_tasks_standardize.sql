...
BEGIN
  ...
  ELSIF p_new_status = 'doing' THEN
    ...
    ELSIF v_task.status = 'done' THEN
      -- SIẾT: chỉ admin/director được reopen done.
      -- TP cùng phòng + creator (cấp dưới) không được mở lại báo cáo đã hoàn thành.
      IF p_comment IS NULL OR length(trim(p_comment)) = 0 THEN
        RAISE EXCEPTION 'Vui lòng nhập lý do trả lại';
      END IF;
      IF NOT v_is_top_admin THEN
        RAISE EXCEPTION 'Chỉ Lãnh đạo/Admin được trả lại báo cáo đã hoàn thành';
      END IF;
      v_is_reopen := TRUE;
