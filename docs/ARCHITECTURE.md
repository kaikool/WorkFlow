...
| Form | `react-hook-form` + `zod` | — | **Có sẵn nhưng module hiện tại chưa dùng**. Khuyến nghị áp dụng cho form > 4 trường (xem §5.4) |
...
**Hiện tại:** dự án dùng `useState` riêng lẻ + validate inline. Có `react-hook-form` + `zod` trong `package.json` nhưng chưa dùng (cơ hội nâng cấp).
...
- `canAccessTasksModule(profile)` — mọi role trừ `driver`/`secretary`
...
- `canReopenDone(profile, task)` — mở lại `done → doing` (CHỈ admin/director — chống vòng lặp "duyệt rồi mở lại" vô hạn)
- `canEditTask(profile, task)` — sửa title/description/priority/due_date (creator + admin/director, không cho khi `canceled/archived`)
- `canDeleteDraft(profile, task, commentCount, attachmentCount)` — escape hatch xoá nháp 10 phút sau tạo (chỉ creator, `status='todo'`, 0 comment user, 0 file)
...
**Phiên bản:** 1.3 — 2026-05-26 (bổ sung 16 helper permissions cho Tasks module: `isHubDepartment`, `canAssignTaskToOthers`, `canRequestReport`, `canTargetCrossDepartment`, `canDelegateTask`, `canApproveReport`, `canRejectSubmission`, `canReopenDone`, `canEditTask`, `canDeleteDraft`, `canApproveExtension`, `canCreateRecurringTemplate`, `canViewTaskAnalytics`, `canViewBranchAnalytics`, …).
